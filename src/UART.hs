...
addr_UART_rbr  = 0x00 :: Word64    -- receiver buffer register (read only)
addr_UART_thr  = 0x00 :: Word64    -- transmitter holding register (write only)
addr_UART_ier  = 0x08 :: Word64    -- interrupt enable register
addr_UART_iir  = 0x10 :: Word64    -- interrupt id register    (read-only)
addr_UART_lcr  = 0x18 :: Word64    -- line control reg
addr_UART_mcr  = 0x20 :: Word64    -- modem control reg
addr_UART_lsr  = 0x28 :: Word64    -- line status reg     (read-only)
addr_UART_msr  = 0x30 :: Word64    -- modem status reg    (read-only)
...
addr_UART_dlm  = 0x08 :: Word64    -- divisor latch high
addr_UART_fcr  = 0x10 :: Word64    -- fifo control reg    (write-only)

-- Bit fields of ier (Interrupt Enable Register)
uart_ier_erbfi = 0x01 :: Word8     -- Enable Received Data Available Interrupt
uart_ier_etbei = 0x02 :: Word8     -- Enable Transmitter Holding Register Empty Interrupt
uart_ier_elsi  = 0x04 :: Word8     -- Enable Receiver Line Status Interrupt
uart_ier_edssi = 0x08 :: Word8     -- Enable Modem Status Interrupt

-- iir values (Interrupt Identification Register) in decreasing priority of interrupts
uart_iir_none  = 0x01 :: Word8     -- None (no interrupts pending)
uart_iir_rls   = 0x06 :: Word8     -- Receiver Line Status
uart_iir_rda   = 0x04 :: Word8     -- Received Data Available
uart_iir_cti   = 0x0C :: Word8     -- Character Timeout Indication
uart_iir_thre  = 0x02 :: Word8     -- Transmitter Holding Register Empty
uart_iir_ms    = 0x00 :: Word8     -- Modem Status
...
-- Bit fields of LSR
uart_lsr_rxfe  = 0x80 :: Word8    -- Receiver FIFO error
uart_lsr_temt  = 0x40 :: Word8    -- Transmitter empty
uart_lsr_thre  = 0x20 :: Word8    -- THR empty
uart_lsr_bi    = 0x10 :: Word8    -- Break Interrupt
uart_lsr_fe    = 0x08 :: Word8    -- Framing Error
uart_lsr_pe    = 0x04 :: Word8    -- Parity Error
uart_lsr_oe    = 0x02 :: Word8    -- Overrun Error
uart_lsr_dr    = 0x01 :: Word8    -- Data Ready
...
data UART_NS16550A = UART_NS16550A {
  ...
  f_ier   :: Word8,     -- 1
  -- f_iir   :: Word8,  -- 2    Virtual read-only field; computed from other fields on each read
  f_lcr   :: Word8,     -- 3
  f_mcr   :: Word8,     -- 4
  f_lsr   :: Word8,     -- 5    Read-only
  f_msr   :: Word8,     -- 6    Read-only
  f_scr   :: Word8,     -- 7


  -- Aliased registers, depending on control bits
  f_dll   :: Word8,    -- 0    if LCR.DLAB=1
  f_dlm   :: Word8,    -- 1    if LCR.DLAB=1

  f_fcr   :: Word8     -- 2    Write-only
  }
...
-- ================================================================
-- Computing the virtual field 'iir'

uart_read_iir :: UART_NS16550A -> Word8
uart_read_iir  uart =
  let
    ier  = f_ier  uart
    lsr  = f_lsr  uart
...
uart_write  uart  addr_offset  val =
  ...
  -- FCR (write-only)
  else if (addr_offset == addr_UART_fcr) then uart { f_fcr = fromIntegral (val .&. 0xFF) }

  else if (addr_offset == addr_UART_lcr) then uart { f_lcr = fromIntegral (val .&. 0xFF) }
  else if (addr_offset == addr_UART_mcr) then uart { f_mcr = fromIntegral (val .&. 0xFF) }
  else if (addr_offset == addr_UART_lsr) then uart    -- LSR is read-only
  else if (addr_offset == addr_UART_msr) then uart    -- MSR is read-only
  else if (addr_offset == addr_UART_scr) then uart { f_scr = fromIntegral (val .&. 0xFF) }
  ...
  -- DLL (alias of IER)
      ...
      uart { f_dlm = fromIntegral (val .&. 0xFF) }
